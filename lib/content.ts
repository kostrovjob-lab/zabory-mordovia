...
const introVariants = [
  "Компания «Заборы Мордовия» выполняет {service} в {city} и прилегающих районах. {region} — мы знаем местные особенности и предлагаем решения, проверенные на сотнях объектов.",
  "Нужен надёжный забор в {city}? Мы специализируемся на {service} и работаем по всей республике. {localFeature} — это наше преимущество перед «общероссийскими» бригадами.",
  "Заказать {service} в {city} можно с бесплатным выездом замерщика. Мы — местная бригада с опытом более 8 лет, {region}. Сами закупаем материалы и сдаём объект под ключ.",
...
const bodyBlocks = [
  ...
  },
  {
    ...
    paragraphs: [
      "Монтаж {service} в {city} занимает от 2 до 5 рабочих дней. Доставка материалов — {deliveryDays} день после подтверждения заказа. {localFeature}.",
